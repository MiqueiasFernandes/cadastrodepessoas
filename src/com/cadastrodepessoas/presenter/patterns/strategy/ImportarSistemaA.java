...
public class ImportarSistemaA implements IStrategyImportar, IColecao<Pessoa> {

    private TreeSet<Pessoa> pessoas;

    public ImportarSistemaA() {
        pessoas = new TreeSet<>();
    }
    ...
    public boolean preparaParaImportar(IStrategyDesktop dektop) throws Exception {
        ...
        if (returnVal == JFileChooser.APPROVE_OPTION) {
            ...
            Class classe
                    = Class.forName("sistemaa.SistemaA", true, classLoader);
            ...
            List colecao = (List) obj;
            Class collaborador = colecao.get(0).getClass();

            Method nome = collaborador.getMethod("getNomeCompleto", new Class[]{});
            Method telefone = collaborador.getMethod("getTelefone", new Class[]{});

            Method metodos[] = {nome, telefone};
    ...
    Pessoa importaCollaborator(Object collaborator, Method[] metodos)
        ...
        String nome = (String) metodos[0].invoke(collaborator, null);
        String tel = (String) metodos[1].invoke(collaborator, null);
